...
# Benchmark Runner for Deno Runtime
...
echo -e "${BLUE}╔════════════════════════════════════════════════╗${NC}"
echo -e "${BLUE}║  Benchmark Suite - TypeScript with Deno        ║${NC}"
echo -e "${BLUE}╚════════════════════════════════════════════════╝${NC}"
echo ""

# Check if deno is installed
if ! command -v deno &> /dev/null; then
    echo -e "${RED}⚠️  Deno is not installed!${NC}"
    echo "Please install Deno: curl -fsSL https://deno.land/install.sh | bash"
    exit 1
fi
...
for i in $(seq 1 $WARMUP_RUNS); do
    deno run --allow-read --allow-write --allow-net main.ts "$SEARCH_WORD" > /dev/null 2>&1 || true
done
echo ""
...
for i in $(seq 1 $RUNS); do
    ...
    if [[ "$OSTYPE" == "linux-gnu"* ]]; then
        ...
        if command -v /usr/bin/time &> /dev/null; then
            /usr/bin/time -v deno run --allow-read --allow-write --allow-net main.ts "$SEARCH_WORD" 2>&1 | tee -a "$RESULTS_FILE"
        else
            time deno run --allow-read --allow-write --allow-net main.ts "$SEARCH_WORD" 2>&1 | tee -a "$RESULTS_FILE"
        fi
