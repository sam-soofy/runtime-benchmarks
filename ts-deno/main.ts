// main.ts - Entry point for Deno Runtime Benchmark
import { runCpuTasks } from './cpu-tasks.ts';
import { runNetworkTasks } from './network-tasks.ts';
import { searchCsvFile } from './file-tasks.ts';
...
async function runBenchmark(): Promise<void> {
  ...
  console.log("🚀 Starting Benchmark (Deno Runtime)");
  ...
  const searchWord = Deno.args[0] || 'example';
